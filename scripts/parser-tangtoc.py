...
def main():
    ...
    try:
        ...
        # In thống kê
        print(f"\nThống kê:", file=sys.stderr)
        print(f"- Tổng câu hỏi hợp lệ: {stats['total_questions']}", file=sys.stderr)
        print(f"- Câu 1 (10s): {stats['question_1']}", file=sys.stderr)
        print(f"- Câu 2 (20s): {stats['question_2']}", file=sys.stderr)
        print(f"- Câu 3 (30s): {stats['question_3']}", file=sys.stderr)
        print(f"- Câu 4 (40s): {stats['question_4']}", file=sys.stderr)
        print(f"- Có ảnh: {stats['with_images']}", file=sys.stderr)
        print(f"- Dòng bị bỏ qua: {stats['skipped_count']}", file=sys.stderr)
        print(f"- Tổng dòng xử lý: {stats['total_processed']}", file=sys.stderr)
        
        if skipped_rows:
            print(f"\nCác dòng bị bỏ qua (hiển thị 5 dòng đầu):", file=sys.stderr)
            for row in skipped_rows[:5]:
                print(f"  - Dòng {row['row']}: {row['reason']}", file=sys.stderr)
