document.addEventListener('DOMContentLoaded', () => {
  ...
  async function loadLogs(page = 1) {
    ...
    const res = await fetch(`/api/admin/question-logs?${params.toString()}`, { credentials: 'include' });
  ...
  function renderTable() {
    ...
    tableBody.innerHTML = filtered.map(log => `
      <tr>
        ...
        <td>
          <div class="question-text">${escapeHtml((log.question_text || '').slice(0, 100))}</div>
  ...
  tableBody.addEventListener('click', async (e) => {
    ...
    const res = await fetch(`/api/admin/question-logs/${id}`, { credentials: 'include' });
  ...
  restoreBtn.addEventListener('click', async () => {
    ...
    const confirmed = confirm('Bạn có chắc chắn muốn khôi phục câu hỏi này không?');
    if (!confirmed) return;
    
    try {
      const res = await fetch(`/api/admin/question-logs/${currentSelectedLogId}/restore`, { 
        method: 'POST', 
        credentials: 'include' 
      });
      ...
      if (result.success) {
        alert('Đã khôi phục câu hỏi thành công');
  ...
  permanentlyDeleteBtn.addEventListener('click', async () => {
    ...
    try {
      const res = await fetch(`/api/admin/question-logs/${currentSelectedLogId}/permanently-delete`, { 
        method: 'POST', 
        credentials: 'include' 
      });
